...
if __name__ == '__main__':
    ...
    ServiceFolder = '\\\\ctc-network.intranet\\dfs\\BPATCR\\Contract-Archive\\Project\\Novartis'
            ...
            for index, row in ServiceDf.iterrows():
                if is_haematology(row[1]) or is_haematology(row[0]):
                    ...
                    if 'coagulation' in row[0].lower():
                        ...
                        print(haematology.match_coagulation_tests(row[0], filtered_sentence))
                    elif 'examination of blood film' in row[0].lower():
                        print(haematology.match_blood_film_examination(row[0], filtered_sentence))
                    elif 'haematology' in row[0].lower() and 'count' in row[1].lower():
                        print(haematology.match_complete_blood_picture(row[0], filtered_sentence))
                    # else:
